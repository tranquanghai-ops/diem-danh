# Kết nối Firebase cho IFA+ Check-in V1.5.5
...
- Cấu hình Web của dự án `ifa-checkin` đã được lưu sẵn trong ứng dụng IFA+ Check-in V1.5.5.
- Đây là cấu hình Web công khai của ứng dụng; **không lấy tệp Service Account hoặc private key**.

## 5. Kết nối ứng dụng

- Mở [trang quản lý](https://tranquanghai-ops.github.io/ifa-checkin/admin/), kiểm tra **IFA+ Check-in V1.5.5**.
...
- V1.5.5 không tải danh sách SV khi Admin đăng nhập. Mở tab chỉ đọc danh mục ngành/lớp nhỏ; sau khi chọn và bấm **Lấy danh sách**, hệ thống mới tải đúng nhóm đã chọn. Nhập file danh sách toàn khoa để tạo chỉ mục nhóm, không cần đọc trước hàng nghìn hồ sơ từ Firebase.
