# Kết nối Firebase cho IFA+ Check-in V1.5.8
...
- Cấu hình Web của dự án `ifa-checkin` đã được lưu sẵn trong ứng dụng IFA+ Check-in V1.5.8.
- Đây là cấu hình Web công khai của ứng dụng; **không lấy tệp Service Account hoặc private key**.

## 5. Kết nối ứng dụng

- Mở [trang quản lý](https://tranquanghai-ops.github.io/ifa-checkin/admin/), kiểm tra **IFA+ Check-in V1.5.8**.
...
- V1.5.8 không tải danh sách SV khi Admin đăng nhập. Giữ **Tất cả ngành** hoặc chọn ngành/lớp rồi bấm **Lấy danh sách**; hệ thống chỉ tải nhóm được yêu cầu. Nút **Tìm kiếm** đọc trực tiếp hồ sơ khi nhập chính xác MSSV, còn tìm theo họ tên sử dụng chỉ mục danh sách đã tạo.
