# Kết nối Firebase cho Điểm danh V1.5.3
...
- Cấu hình Web của dự án `diem-danh-tknt` đã được lưu sẵn trong ứng dụng V1.5.3.
- Đây là cấu hình Web công khai của ứng dụng; **không lấy tệp Service Account hoặc private key**.

## 5. Kết nối ứng dụng

- Mở [trang quản lý](https://tranquanghai-ops.github.io/diem-danh/admin/), kiểm tra **V1.5.3**.
...
- V1.5.3 không tải danh sách SV khi Admin đăng nhập. Mở tab chỉ đọc danh mục ngành/lớp nhỏ; sau khi chọn và bấm **Lấy danh sách**, hệ thống mới tải đúng nhóm đã chọn. Nhập lại file danh sách toàn khoa một lần để tạo chỉ mục nhóm, không cần đọc trước hàng nghìn hồ sơ cũ từ Firebase.
