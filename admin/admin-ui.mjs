...
let editing=false,viewerScale=1,newRoster=[],studentPage=1,rosterPage=1,attendancePage=1,rosterFilter='';
...
function renderRoster(){
  ...
  const allRows=[...cloud.roster.map(r=>({...r,state:attended.has(r.mssv)?'Có mặt':'Vắng'})),...c.outside.map(r=>({mssv:r.mssv,name:cloud.displayStudentName(r.mssv,r.studentName),state:'Ngoài danh sách'}))],filtered=rosterFilter==='present'?allRows.filter(r=>r.state==='Có mặt'):rosterFilter==='absent'?allRows.filter(r=>r.state==='Vắng'):rosterFilter==='outside'?allRows.filter(r=>r.state==='Ngoài danh sách'):allRows,pageSize=Number($('rosterPageSize').value)||10,totalPages=Math.max(1,Math.ceil(filtered.length/pageSize));rosterPage=Math.min(Math.max(1,rosterPage),totalPages);const rows=filtered.slice((rosterPage-1)*pageSize,rosterPage*pageSize);
  ...
  document.querySelectorAll('[data-roster-filter]').forEach(b=>b.setAttribute('aria-pressed',String(b.dataset.rosterFilter===rosterFilter)));$('rosterPagination').hidden=filtered.length<=pageSize;$('rosterPageInfo').textContent=`Trang ${rosterPage}/${totalPages} • ${filtered.length} dòng`;$('rosterPrevPage').disabled=rosterPage<=1;$('rosterNextPage').disabled=rosterPage>=totalPages;
}
...
function render(){
  ...
  const photosById=new Map(cloud.photos.map(p=>[p.id,p])),rows=cloud.rows||[],attendancePageSize=Number($('attendancePageSize').value)||10,attendanceTotalPages=Math.max(1,Math.ceil(rows.length/attendancePageSize));attendancePage=Math.min(Math.max(1,attendancePage),attendanceTotalPages);const attendanceRows=[...rows].reverse().slice((attendancePage-1)*attendancePageSize,attendancePage*attendancePageSize);$('savedCount').textContent=rows.length+' lượt';$('empty').hidden=!!rows.length;
  $('rows').innerHTML=attendanceRows.map((r,i)=>{const image=r.photoId&&photosById.has(r.photoId)?`<button class="photo-link" data-view-photo="${esc(r.photoId)}" title="Xem hình"><b>${esc(r.mssv)}</b> 🖼️</button>`:`<b>${esc(r.mssv)}</b>`,stt=rows.length-((attendancePage-1)*attendancePageSize+i);return `<tr><td class="stt-col">${stt}</td><td class="mssv-col">${image}</td><td class="student-col">${esc(cloud.displayStudentName(r.mssv,r.studentName))}</td><td class="member-col">${esc(r.memberName||'Không rõ')}</td><td class="delete-col"><button class="danger" data-delete-scan="${esc(r.mssv)}">Xóa</button></td><td class="time-col">${esc(time(r.time))}</td><td class="event-col">${esc(r.eventName||cloud.eventName)}</td></tr>`;}).join('');
  $('attendancePagination').hidden=rows.length<=attendancePageSize;$('attendancePageInfo').textContent=`Trang ${attendancePage}/${attendanceTotalPages} • ${rows.length} lượt`;$('attendancePrevPage').disabled=attendancePage<=1;$('attendanceNextPage').disabled=attendancePage>=attendanceTotalPages;
...
$('eventArchiveRows').onclick=async e=>{const b=e.target.closest('[data-open-event]');if(!b)return;editing=false;rosterPage=1;attendancePage=1;rosterFilter='';await action(()=>cloud.loadEvent(b.dataset.eventDay,b.dataset.openEvent))();};
for(const id of ['eventName','eventLocation','eventStartTime','eventEndTime','members'])$(id).oninput=()=>{editing=true;};

$('newEventDay').value=vietnamDay();$('newEventDay').min=vietnamDay();$('newEventDay').max=plusDays(vietnamDay(),10);$('newEndDay').value=plusDays(vietnamDay(),2);$('newEndDay').min=vietnamDay();$('newEndDay').max=plusDays(vietnamDay(),5);
$('newEventDay').onchange=()=>{const day=$('newEventDay').value||vietnamDay();$('newEndDay').min=day;$('newEndDay').max=plusDays(day,5);$('newEndDay').value=plusDays(day,2);};
...
$('rosterFile').onchange=async()=>{const file=$('rosterFile').files[0];if(!file)return;await action(async()=>{const rows=parseStudents(await file.arrayBuffer());if(!confirm(`Tìm thấy ${rows.length} MSSV hợp lệ. Gộp vào danh sách đăng ký hiện tại?`))return;await cloud.mergeRoster(rows);})();$('rosterFile').value='';};
...
$('studentPrevPage').onclick=()=>{if(studentPage>1){studentPage--;renderStudents();}};$('studentNextPage').onclick=()=>{studentPage++;renderStudents();};
...
$('exportStudentsBtn').onclick=action(async()=>{const records=await cloud.loadAllIndexedStudents(),rows=records.map((x,i)=>({STT:i+1,'Mã SV':x.mssv,'Họ và tên':x.name,'Giới tính':x.gender||'','Ngành':x.major||'','Lớp':x.studentClass||''})),ws=XLSX.utils.json_to_sheet(rows),wb=XLSX.utils.book_new();ws['!cols']=[{wch:7},{wch:16},{wch:34},{wch:12},{wch:28},{wch:18}];XLSX.utils.book_append_sheet(wb,ws,'Danh sach SV khoa');XLSX.writeFile(wb,'DSSV_KHOA_'+vietnamDay()+'.xlsx');});

document.querySelectorAll('[data-roster-filter]').forEach(b=>b.onclick=()=>{rosterFilter=rosterFilter===b.dataset.rosterFilter?'':b.dataset.rosterFilter;rosterPage=1;renderRoster();});
$('rosterPageSize').onchange=()=>{rosterPage=1;renderRoster();};$('rosterPrevPage').onclick=()=>{if(rosterPage>1){rosterPage--;renderRoster();}};$('rosterNextPage').onclick=()=>{rosterPage++;renderRoster();};
$('attendancePageSize').onchange=()=>{attendancePage=1;render();};$('attendancePrevPage').onclick=()=>{if(attendancePage>1){attendancePage--;render();}};$('attendanceNextPage').onclick=()=>{attendancePage++;render();};
$('exportRosterBtn').onclick=()=>{const attended=new Map(cloud.rows.map(r=>[r.mssv,r])),rosterIds=new Set(cloud.roster.map(r=>r.mssv)),registered=cloud.roster.map((r,i)=>({STT:i+1,MSSV:r.mssv,'Họ tên':r.name,'Đối chiếu':attended.has(r.mssv)?'Có mặt':'Vắng','Thời gian điểm danh':attended.has(r.mssv)?time(attended.get(r.mssv).time):''})),outside=cloud.rows.filter(r=>!rosterIds.has(r.mssv)).map((r,i)=>({STT:registered.length+i+1,MSSV:r.mssv,'Họ tên':cloud.displayStudentName(r.mssv,r.studentName),'Đối chiếu':'Ngoài danh sách','Thời gian điểm danh':time(r.time)})),ws=XLSX.utils.json_to_sheet([...registered,...outside]),wb=XLSX.utils.book_new();ws['!cols']=[{wch:7},{wch:16},{wch:34},{wch:20},{wch:24}];XLSX.utils.book_append_sheet(wb,ws,'Doi chieu tham du');XLSX.writeFile(wb,safe(cloud.eventName)+'_DOI_CHIEU_'+cloud.day+'.xlsx');};

document.querySelector('[data-nav="students"]').addEventListener('click',()=>void action(()=>cloud.loadStudentFilters())());
document.querySelector('[data-nav="events"]').addEventListener('click',()=>{editing=false;rosterPage=1;attendancePage=1;rosterFilter='';cloud.clearSelectedEvent();});
