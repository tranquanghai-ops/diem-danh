...
function render(){
  ...
  $('rows').innerHTML=[...rows].reverse().map((r,i)=>`<tr><td class="stt-col">${rows.length-i}</td><td class="mssv-col"><b>${esc(r.mssv)}</b></td><td class="member-col">${esc(r.memberName||'Không rõ')}</td><td class="delete-col"><button class="danger" data-delete-scan="${esc(r.mssv)}">Xóa</button></td><td class="time-col">${esc(time(r.time))}</td><td class="event-col">${esc(r.eventName||cloud.eventName)}</td></tr>`).join('');
