...
$('verifyMemberBtn').onclick=run(async()=>{
  const name=$('scannerName').value.trim(),result=await cloud.verifyMember(name);ui.setStatus('success',result.owner?'✓ Đã xác nhận giảng viên.':result.delegated?'✓ Đã cấp quyền quản lý phụ.':'✓ Sẵn sàng quét.',cloud.eventName);
},'Không xác nhận được tên');
$('changeMemberBtn').onclick=run(async()=>{ui.stopScanner();await cloud.resetMember();$('scannerName').value='';setTimeout(()=>$('scannerName').focus(),50);ui.setStatus('','Nhập lại tên người quét.','Tên và quyền cũ đã được đóng.');},'Không đổi được tên');
